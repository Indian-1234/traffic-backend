...
def create_bell_state_circuit():
    ...
    # Apply CNOT gate with control=first qubit, target=second qubit
    circuit.cnot(0, 1)
...
def create_quantum_fourier_transform_circuit(n_qubits=3):
    ...
    QFT is a key component in many quantum algorithms like Shor's algorithm.
    """
    circuit = Circuit()
    ...
    for i in range(n_qubits):
        for j in range(i+1, n_qubits):
            # Calculate phase angle based on qubit distance
